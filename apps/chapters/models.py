...
def apply_lock_from_chapter(story):
    """Set is_locked on all chapters of a story based on story.lock_from_chapter.
    lock_from_chapter=None → all unlocked.
    lock_from_chapter=5    → chapters 1-4 unlocked, 5+ locked.
    """
    lock_from = story.lock_from_chapter
    if lock_from is None:
        # No locking — unlock everything
        Chapter.objects.filter(story=story).update(is_locked=False)
    else:
        Chapter.objects.filter(story=story, chapter_number__lt=lock_from).update(is_locked=False)
        Chapter.objects.filter(story=story, chapter_number__gte=lock_from).update(is_locked=True)
...
class Chapter(models.Model):
    ...
    def publish_held_chapters_for_author(cls, author):
        ...
        # Push notifications to bookmarkers for each story that had chapters published
        if story_ids and published_count > 0:
            try:
                from apps.notifications.services import on_new_chapter as _on_new_chapter
                for sid in story_ids:
                    s = Story.objects.filter(pk=sid).first()
                    if not s:
                        continue
                    latest_ch = cls.objects.filter(
                        story=s, is_published=True
                    ).order_by('-chapter_number').first()
                    if latest_ch:
                        _on_new_chapter(s, latest_ch.title)
            except Exception:
                pass
