...
def login_page(request):
    ...
    next_url = request.GET.get('next', '/dashboard/')
    response = render(request, 'novelux/login.html', {
        'next': next_url,
...
def logout_view(request):
    response = redirect('novelux:login')
    _clear_auth_cookies(response)
    response['Cache-Control'] = 'no-store, no-cache, must-revalidate'
    return response
...
def _editorial_context(request):
    ...
    if role == 'se':
        ...
        # Stories already SE-approved (passed to CE) — includes awaiting_signature and signed
        approved_stories = Story.objects.filter(
            contract_status__in=['contract_sent', 'awaiting_signature', 'signed'],
            author__editor_link__assigned_se=user,
        ).select_related('author').prefetch_related('contract_application').order_by('-updated_at')
        # Attach full chapter list to each approved story so SE can browse chapters
        ...
        _month_start = _date.today().replace(day=1)
        new_authors_month = _User.objects.filter(
            role='author',
            editor_link__assigned_se=user,
            date_joined__date__gte=_month_start,
        ).count()
        signed_count = Story.objects.filter(
            contract_status='signed',
            author__editor_link__assigned_se=user,
        ).count()
        awaiting_sign_count = Story.objects.filter(
        ...
        # Chart 1: SE's authors' books — top 10 by views
        _author_stories_qs = Story.objects.filter(
            author__editor_link__assigned_se=user,
        ).order_by('-total_views').values(
            'title', 'total_views', 'total_chapters', 'word_count', 'status', 'author__username',
        )[:10]
        author_books_chart_data = _json.dumps([
            {
                'title': s['title'][:28] + '…' if len(s['title']) > 28 else s['title'],
                'views': s['total_views'],
                'chapters': s['total_chapters'],
                'words': s['word_count'],
                'status': s['status'],
                'author': s['author__username'],
            }
            for s in _author_stories_qs
        ])

        # Chart 2a: Platform — top 10 stories by views
        _top_platform_qs = Story.objects.order_by('-total_views').values(
            'title', 'total_views', 'author__username',
        )[:10]
        top_platform_stories_data = _json.dumps([
            {
                'title': s['title'][:28] + '…' if len(s['title']) > 28 else s['title'],
                'views': s['total_views'],
                'author': s['author__username'],
            }
            for s in _top_platform_qs
        ])

        # Chart 2b: Platform — story status distribution
        _status_qs = Story.objects.values('status').annotate(count=_DCount('id')).order_by('-count')
        platform_status_data = _json.dumps([
            {'status': s['status'].replace('_', ' ').title(), 'count': s['count']}
            for s in _status_qs
        ])
...
def articles_page(request):
    """Public articles page — shows all published public-audience blog posts."""
    return render(request, 'novelux/articles.html', {
        'access_token': _jwt_from_request(request),
    })


def _build_stories_list(request, qs):
    """Shared helper: convert a Story queryset to a list of dicts for the explore page."""
    result = []
    for s in qs:
        try:
        ...
        result.append({
            ...
            'synopsis':        s.synopsis or (s.description[:300] if s.description else ''),
            'cover_url':       cover,
...
def _strip_chapter_html(html):
    """Strip HTML from chapter content while preserving paragraph structure."""
    import re
    html = re.sub(r'<(?:p|div|br|h[1-6])[^>]*/?>', '\n\n', html, flags=re.IGNORECASE)
    html = re.sub(r'</(?:p|div|h[1-6])>', '', html, flags=re.IGNORECASE)
    html = re.sub(r'<[^>]+>', '', html)
...
def _build_explore_sections(request):
    ...
    # Editor's Picks
    add_section("Editor's Picks", '', Story.objects.filter(
        status__in=PUB, is_editors_pick=True,
    ).select_related('author', 'genre').order_by('-total_views'))

    # Featured
    add_section('Featured Stories', '', Story.objects.filter(
        status__in=PUB, is_featured=True,
    ).select_related('author', 'genre').order_by('-total_views'))

    # Most Popular
    add_section('Most Popular', '', Story.objects.filter(
        status__in=PUB,
    ).select_related('author', 'genre').order_by('-total_views'))

    # New Releases
    add_section('New Releases', '', Story.objects.filter(
        status__in=PUB,
...
def explore_stories(request):
    """Public story explorer — sectioned home view with one free chapter preview."""
    import json as _json

    sections, all_stories, genres = _build_explore_sections(request)

    return render(request, 'novelux/explore.html', {
        'sections_json': _json.dumps(sections, ensure_ascii=False),
        'stories_json':  _json.dumps(all_stories, ensure_ascii=False),
        'genres':        genres,
        'open_slug':     None,
        'og':            {},
    })
...
def story_preview(request, slug):
    """SEO permalink for a single story — /explore/<slug>/"""
    import re, json as _json
    from apps.stories.models import Story, Genre
    from apps.chapters.models import Chapter

    try:
        story = Story.objects.select_related('author', 'genre').get(
    ...
    try:
        ...
        if ch and ch.content:
            chapter_excerpt = _strip_chapter_html(ch.content)
            chapter_title   = ch.title or f'Chapter {ch.chapter_number}'
    except Exception:
        pass

    try:
        cover_abs = request.build_absolute_uri(story.cover_image.url) if story.cover_image else ''
    except Exception:
        cover_abs = ''
    ...
    # Inject full chapter content into the matching story entry
    for s in all_stories:
        if s['slug'] == slug:
            s['chapter_excerpt'] = chapter_excerpt
...
def explore_chapter_api(request, slug):
    ...
    try:
        ...
        if ch and ch.content:
            content = _strip_chapter_html(ch.content)
            return JsonResponse({
...
def article_detail(request, slug):
    """Public permalink for a single article — /articles/<slug>/
    Renders the same articles page but tells JS to open the article immediately."""
