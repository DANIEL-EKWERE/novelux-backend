...
class Announcement(models.Model):
    ...
    TARGET_CHOICES = [
        (TARGET_ALL,     'Everyone'),
        (TARGET_AUTHORS, 'Authors only'),
        (TARGET_SES,     'Senior Editors only'),
    ]
...
class AuthorWarning(models.Model):
    REASON_PLAGIARISM = 'plagiarism'
    REASON_AI_CONTENT = 'ai_content'
    REASON_MISCONDUCT = 'misconduct'
    REASON_INACTIVITY = 'inactivity'
    REASON_OTHER      = 'other'
    REASON_CHOICES = [
        (REASON_PLAGIARISM, 'Plagiarism'),
        (REASON_AI_CONTENT, 'AI-Generated Content'),
        (REASON_MISCONDUCT, 'Misconduct'),
        (REASON_INACTIVITY, 'Inactivity'),
        (REASON_OTHER,      'Other'),
    ]

    author     = models.ForeignKey(User, on_delete=models.CASCADE, related_name='warnings',
                                   limit_choices_to={'role': 'author'})
    ...
    class Meta:
        db_table = 'author_warnings'
        ordering = ['-created_at']

    def __str__(self):
        return f'Warning for {self.author.username}: {self.reason}'
...
class SEPromotionRequest(models.Model):
    STATUS_PENDING  = 'pending'
    ...
    se          = models.ForeignKey(User, on_delete=models.CASCADE, related_name='promotion_requests',
                                    limit_choices_to={'role': 'se'})
    story       = models.ForeignKey('stories.Story', on_delete=models.CASCADE, related_name='promotion_requests')
    tab         = models.CharField(max_length=20, choices=TAB_CHOICES)
    section     = models.CharField(max_length=50, help_text='Section slug, e.g. picks-for-you, fresh-reads')
    ...
    class Meta:
        db_table = 'se_promotion_requests'
        ordering = ['-created_at']

    def __str__(self):
        return f'{self.se.username} → [{self.tab}/{self.section}] {self.story.title}'


# ─── Explore Tab Pins (approved / CE-direct promotions) ──────────────────────

class ExploreTabPin(models.Model):
    """
    A story actively pinned to a specific explore tab section.
    Created either when a CE approves an SEPromotionRequest, or directly by a CE.
    Pinned stories are prepended to the section's story list.
    """
    tab            = models.CharField(max_length=20, choices=TAB_CHOICES, db_index=True)
    section        = models.CharField(max_length=50, db_index=True)
    story          = models.ForeignKey('stories.Story', on_delete=models.CASCADE, related_name='tab_pins')
    pinned_by      = models.ForeignKey(User, on_delete=models.CASCADE, related_name='tab_pins_added',
                                       limit_choices_to={'role': 'ce'})
    source_request = models.OneToOneField(SEPromotionRequest, on_delete=models.SET_NULL,
                                          null=True, blank=True, related_name='resulting_pin')
...
class AuthorMessage(models.Model):
    ...
    body       = models.TextField()
    msg_type   = models.CharField(max_length=10, choices=TYPE_CHOICES, default=TYPE_OTHER)
...
class SystemNotice(models.Model):
    ...
    TYPE_CHOICES = [
        ...
        (TYPE_ERROR,   'Error/Alert (red)'),
        (TYPE_SUCCESS, 'Success (green)'),
    ]

    message   = models.TextField(help_text='Message shown across all dashboards.')
    notice_type = models.CharField(max_length=10, choices=TYPE_CHOICES, default=TYPE_INFO)
    cta_label = models.CharField(max_length=80, blank=True, default='', help_text='Optional button label.')
    cta_url   = models.CharField(max_length=300, blank=True, default='', help_text='Optional button URL.')
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f'[{self.notice_type.upper()}] {self.message[:60]}'
